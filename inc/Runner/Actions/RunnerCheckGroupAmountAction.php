...
namespace AM\Scheduler\Runner\Actions;

use AM\Scheduler\Base\Traits\Singleton;
use AM\Scheduler\Groups\Controllers\GroupsController;
use AM\Scheduler\Integrations\MakeCom\MakeComCli;
use AM\Scheduler\Runner\Abstractions\RunnerActionAbstraction;
use AM\Scheduler\Runner\Interfaces\RunnerActionInterface;
use WC_Points_Rewards_Manager;

class RunnerCheckGroupAmountAction extends RunnerActionAbstraction implements
    RunnerActionInterface
{
    use Singleton;
    ...
    public function onCall(array $series = []): void
    {
        ...
        if (!empty($series)) {
            foreach ($series as $series_item) {
                $members = !empty($series_item["members_ids"])
                    ? json_decode($series_item["members_ids"])
                    : [];
                $group_id = !empty($series_item["group_id"])
                    ? $series_item["group_id"]
                    : null;
                $group_owner = !empty($group_id)
                    ? GroupsController::getOwner($group_id)
                    : null;
                $amount_for_each_member = !empty($series_item["amount"])
                    ? $series_item["amount"]
                    : 0;

                $collected_members = !empty(
                    $collected[$group_owner["ID"]]["members"]
                )
                    ? $collected[$group_owner["ID"]]["members"]
                    : [];
                $collected_amount = !empty(
                    $collected[$group_owner["ID"]]["amount_for_each_member"]
                )
                    ? $collected[$group_owner["ID"]]["amount_for_each_member"]
                    : 0;
                ...
                $collected[$group_owner["ID"]] = [
                    "members" => array_unique(
                        array_merge($collected_members, $members)
                    ),
                    "amount_for_each_member" =>
                        $amount_for_each_member + $collected_amount,
                    "group_id" => $group_id,
                ];
            }
        }
        if (!empty($collected)) {
            foreach ($collected as $group_owner_id => $data) {
                ...
                $group_points = WC_Points_Rewards_Manager::get_users_points(
                    $group_owner_id
                );
                $needed_amount =
                    $data["amount_for_each_member"] * count($data["members"]);
